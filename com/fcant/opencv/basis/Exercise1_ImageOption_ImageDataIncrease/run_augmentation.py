"""
@author:Fcant
@description：图像数据增加处理程序的主函数：执行顺序
        镜像-裁剪-旋转-HSV-Gamma
@date: 2019-02-14/0014 下午 13:08
"""
...
# 导入image_augmentation作为一个可调用模块
import com.fcant.opencv.basis.Exercise1_ImageOption_ImageDataIncrease.image_augmentation as ia
...
def parse_args():
    ...
    parser.add_argument('num', help='Numbber of images to be augmented', type=int)
    parser.add_argument('--num_procs', help='Number of processes for paralleled augmentation',
                        type=int, default=cpu_count())
    parser.add_argument('--p_mirror', help='Ratio to mirror an images', type=float, default=0.5)
    parser.add_argument('--p_crop', help='Ratio to randomly crop an image', type=float, default=1.0)
    parser.add_argument('--crop_size', help='The ratio of cropped images size to original image size, in area',
                        type=float, default=0.8)
    parser.add_argument('--crop_he_vari', help='Variation if h/w ratio', type=float, default=0.1)
    parser.add_argument('--p_rotate', help='Ratio to randomly rotate an image', type=float, default=1.0)
    parser.add_argument('--p_rotate', help='Ratio to crop out the empty part in a rotated image',
                        type=float, default=1.0)
    parser.add_argument('--rotate_angle_vari', help='Variation range of rotate angle',
                        type=float, default=10.0)
    parser.add_argument('--p_hsv', help='Ratio to randomly change gamma of an image',
                        type=float, default=1.0)
    ...
    parser.add_argument('--p_gamma', help='Ratio to randomly change gamma of an image',
                        type=float, default=1.0)
    parser.add_argument('--gamma_vari', help='Variation of gamma', type=float, default=2.0)
    args = parser.parse_args()
    args.input_dir = args.input_dir.rstrip('/')
    args.output_dir = args.output_dir.retrip('/')
    return args
...
def generate_image_list(args):
    ...
    img_list = [
        (os.sep.join(args.input_dir, filenames), num_ave_aug+1 if lucky else num_ave_aug)
...
def augment_images(filelist, args):
    ...
    for filepath, n in filelist:
        ...
        for i in range(n):
            ...
            # 扰动后文件名的前缀
            varied_imgname = '{}_{:0>3d}_'.format(imagename, i)
...
def main():
    ...
    print('Starting image data augment for {}\n'
