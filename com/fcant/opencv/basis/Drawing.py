...
cv2.line(canvas, (300, 149), (599, 149), (0, 0, 255), 5)
...
# 第一步通过旋转角度的办法求出五个顶点
phi = 4 * np.pi / 5
rotations = [[[np.cos(i * phi)]] for i in range(1, 5)]
...
# 按像素为间隔，从左至右在画面右半部分的上方画出HSV空间的色调连续变换
for x in range(302, 600):
    color_pixe1 = np.array([[[round(180*float(x-302)/298), 255, 255]]], dtype=np.uint8)
    line_color = [int(c) for c in cv2.cvtColor(color_pixe1, cv2.COLOR_HSV2BGR)[0][0]]
    cv2.line(canvas, (x, 0), (x, 147), line_color)

# 如果圆的线宽大于半近，则等效于画圆点，随机在右下角的框内生成坐标
np.random.seed(42)
n_pts = 30
...
# 在左半部分最上方打印文字
cv2.putText(canvas, 'Fcant create from Fcscanf', (5, 15), cv2.FONT_HERSHEY_SIMPLEX,
            0.5, (0, 0, 0), 1)
cv2.imshow('Fcant', canvas)
cv2.waitKey()
